...
//var slaveObjName = masterObjName;
var slaveRandomNo = (new Date()).getTime();
var slaveObjName = 'ZAR_' + slaveRandomNo;
...
var _QCCALCOBJECT_ZARSPOTSWAP_REQUESTID_CLIENT = _QCCALCOBJECT_ZARSPOTSWAP_REQUESTID + 11;
var _QCCALCOBJECT_ZARFORWARDS_REQUESTID = 31;
var _QCCALCOBJECT_ZARFORWARDS_REQUESTID_CLIENT = _QCCALCOBJECT_ZARFORWARDS_REQUESTID + 11;
...
$(document).ready(function () {
    ...
    addUseCheckboxEventHandlers();
});

function initImpliedIds() {
    for (var k = 0; k < dataSheets.length; k++) {
        for (var l = 0; l < dataSheets[k].length; l++) {
            $(jq(masterObjName + '.' + dataSheets[k][l][0] + '_c')).attr('id', slaveObjName + '.' + dataSheets[k][l][0]);
        }
    }
}
...
function setupWs() {
    ws = new WebSocket(_QCCALCOBJECTENDPOINT);
    ...
    ws.onopen = function () {
        ...
        // SLAVE init
        swap = createCalcSwapReq(slaveObjName);
        console.log('Slave Swap send over socket: ' + swap);
        ws.send(swap);

        fwd = createCalcForwardReq(slaveObjName);
        console.log('Slave Fwd send over socket: ' + fwd);
        ws.send(fwd);

        fwdMatrix = createForwardMatrixReq(slaveObjName);
        console.log('Slave Fwd matrix send over socket: ' + fwdMatrix);
        ws.send(fwdMatrix);
    };
        ...
        if (servermsg.charAt(0) === '{') {
            ...
            $.each(json, function (key, value) {
                if (value === 'updateobject') {
                    if (key === masterObjName || key === slaveObjName) {
                        zarCalcObject(ws, key);
            ...
            else if (requestid === _QCCALCOBJECT_ZARSPOTSWAP_REQUESTID_CLIENT)
                addSpotChart(json);
            else if (requestid === _QCCALCOBJECT_ZARFORWARDS_REQUESTID)
                renderFwdChart(json);
            else if (requestid === _QCCALCOBJECT_ZARFORWARDS_REQUESTID_CLIENT) {
                addFwdChart(json);
            }
        }
    };
...
function createForwardMatrixReq(objName) {
    var i, j;
    var json;
    ...
    for (i = 0; i < zarforwards_instr.length; i++) {
        for (j = 1; j < zarforwards_instr.length; j++) {
            json += '[\'' + objName + '\',\'swap\',\'' + zarforwards_instr[i] + '\',\'' + zarforwards_instr[j] + '\']';
...
function zarCalcObject(ws, name) {
    ...
    var calcFwdMatrixReq = createForwardMatrixReq(objname);
...
function addUseCheckboxEventHandlers() {
...
function sendCreateObject(data) {
    ...
    $.post(_QCCREATEOBJECTENDPOINT, JSON.stringify(data), function (data) {
        ...
        var values = createInitialValuesReq(slaveObjName);
        console.log('Initial values request send over socket: ' + values);
        ws.send(values);
    }, 'json');
}
...
function createParameterArrayFromSourceArrays(dataArray, dataHeaderArray) {
    ...
    if (masterObjName === slaveObjName) {
        slaveObjName = 'ZAR_' + (new Date()).getTime();
    }
    defaultHeaderSheetData[1][1] = slaveObjName;
    // add header data
    ...
    for (j = 0; j < dataArray.length; j++) {
        ...
        for (i = 0; i < dataArray[j].length; i++) {
            ...
            var liveValue = parseFloat($(compositeName).text());
            if (overwriteVal) {
                var relOverwrite = getRelativeValue(overwriteVal);
                if (relOverwrite) {
                    console.log('found relative overwrite:' + relOverwrite + ' live value:' + liveValue);
                    overwriteVal = parseFloat($(compositeName).text()) + (relOverwrite / 100);
                }
                console.log('overWrite ' + compositeName + ' with: ' + overwriteVal);
                row[2] = formatPercentToFloat(overwriteVal, 10);
            } else {
                row[2] = formatPercentToFloat(liveValue, 10);
            }
...
function createCalcSwapReq(objName) {
    var reqId;
    if (objName === 'ZAR') {
        reqId = _QCCALCOBJECT_ZARSPOTSWAP_REQUESTID;
    } else {
        reqId = _QCCALCOBJECT_ZARSPOTSWAP_REQUESTID_CLIENT;
    }
...
function createCalcForwardReq(objName) {
    var reqId;
    if (objName === 'ZAR') {
        reqId = _QCCALCOBJECT_ZARFORWARDS_REQUESTID;
    } else {
        reqId = _QCCALCOBJECT_ZARFORWARDS_REQUESTID_CLIENT;
    }
...
var clientSwaprates;
var lifeSwapRates;
function addSpotChart(json) {
    var swaprates = [];
    for (var i = 0; i < json.length; i++) {
        swaprates[i] = json[i][1];
    }
    clientSwaprates = swaprates;

    renderSpotSwapCharts(lifeSwapRates, clientSwaprates);
}
...
function renderSpotSwapChart(json) {
    ...
    lifeSwapRates = swaprates;

    renderSpotSwapCharts(lifeSwapRates, clientSwaprates);
}

function renderSpotSwapCharts(lifeRates, clientRates) {
    $('#firstRates').highcharts({
        ...
        series: [
            ...
            {
                name: 'Overridden Spot Swap Rates',
                data: clientRates
            }
        ]
    });
...
var lifeforwardRates;
var clientForwardRates;

function addFwdChart(json) {
    var forwardRates = [];
    for (var i = 0; i < json.length; i++) {
        forwardRates[i] = json[i][1];
    }
    clientForwardRates = forwardRates;

    renderFwdCharts(lifeforwardRates, clientForwardRates);
}
...
function renderFwdChart(json) {
    ...
    lifeforwardRates = forwardRates;

    renderFwdCharts(lifeforwardRates, clientForwardRates);
}

function renderFwdCharts(lifeRates, clientRates) {
    $('#thirdRates').highcharts({
        ...
        series: [
            ...
            {
                name: 'Overridden 3M Forward Rates',
                data: clientRates
            }
        ]
    });
...
function zarDisplayCurve(json_rates) {
    ...
    for (i = 0; i < json_rates.length; i++) {
        var name = masterObjName + '.' + zarcurve_instr[i];
        if ((json_rates[i][0] === 1) && !isOverwritten(name)) {
            updateValue(name, json_rates[i][1]);
        }
    }
}
...
function isOverwritten(name) {
    return !!getOverwriteValue(name);
}

function getOverwriteValue(name) {
    var compositeName = jq(name);
    var firstSibling = $(compositeName).next();
    return firstSibling.find('input').val();
}